...
export default function MockupLanding({ className = "" }: { className?: string }) {
  ...
  return (
    <div
      className={`relative w-[270px] rounded-[2.4rem] border border-grafite/10 bg-grafite p-2.5 shadow-glow ${className}`}
    >
      {/* notch */}
        ...
        <div className="px-4 py-4">
          ...
          <div className="space-y-2">
            {servicos.map((s) => (
              <div
                key={s.nome}
                className="flex items-center justify-between rounded-xl border border-rose/50 bg-cream px-3 py-2.5"
              >
